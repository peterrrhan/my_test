---
title: "INFSCI 2595 Spring 2024 Homework: 06"
subtitle: "Assigned February 28, 2024; Due: March 6, 2024"
author: "Xuxin Han"
date: "Submission time: March 5, 2024 at 11:00PM EST"
...
test_values_1 <- c(13, 5)
log_post_1 <- my_logpost(test_values_1, hw06_info)
print(log_post_1)
...
test_values_2 <- c(7, 1.5)
log_post_2 <- my_logpost(test_values_2, hw06_info)
print(log_post_2) 
...
mu_grid_lwr <- mu0 - 3 * tau0
mu_grid_upr <- mu0 + 3 * tau0

sigma_grid_lwr <- 1
lambda <- 0.5
sigma_grid_upr <- qnorm(0.99, mu0, tau0)
```
...
param_grid <- expand.grid(
                          mu = seq(from = mu_grid_lwr, to = mu_grid_upr, length.out = 251),
...
eval_logpost <- function(unknown_1, unknown_2, logpost_func, logpost_info)
{
 unknowns <- c(unknown_1, unknown_2)
  ...
  result <- logpost_func(unknowns, logpost_info)
...
logpost_value <- eval_logpost(mu_test, sigma_test, my_logpost, hw06_info)
...
To determine the second derivative of the log-posterior with respect to the unknown mean, \(\mu\), we start from the log-posterior expression given the observations \(\mathbf{x}\) and fixed likelihood noise \(\sigma = 3\). The log-posterior is the sum of the log-likelihoods for each observation under a Gaussian model and the log-prior for \(\mu\) also under a Gaussian model:

\[
\log p\left( \mu \mid \mathbf{x}, \sigma \right) = \sum_{n=1}^{N} \log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) + \log \left( \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \right)
\]

Substituting the expressions for the Gaussian densities, we obtain:

\[
\log p\left( \mu \mid \mathbf{x}, \sigma \right) = -\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_n - \mu)^2 - \frac{1}{2\tau_0^2} (\mu - \mu_0)^2 + \text{constant}
\]

The first derivative with respect to \(\mu\) is:

\[
\frac{d}{d\mu} \log p\left( \mu \mid \mathbf{x}, \sigma \right) = \frac{1}{\sigma^2} \sum_{n=1}^{N} (x_n - \mu) - \frac{1}{\tau_0^2} (\mu - \mu_0)
\]

Taking the second derivative with respect to \(\mu\), we find:

\[
\frac{d^2}{d\mu^2} \log p\left( \mu \mid \mathbf{x}, \sigma \right) = -\frac{N}{\sigma^2} - \frac{1}{\tau_0^2}
\]

This expression represents the curvature of the log-posterior with respect to \(\mu\), and is essential for the Laplace Approximation step where we approximate the variance of the posterior distribution of \(\mu\).
...
To confirm that the mode corresponds to the maximum log-posterior density and not the minimum, we can verify that the second derivative of the log-posterior with respect to \(\mu\) at the mode is negative, indicating a concave function with a peak at the mode. This ensures that the identified mode represents a local maximum in the log-posterior landscape, as required for a maximum a posteriori (MAP) estimate.
...
The Laplace Approximation approximates the posterior distribution as a Gaussian distribution:

\[
p\left( \mu \mid \mathbf{x}, \sigma \right) \approx \mathrm{normal} \left( \mu \mid \mathrm{m}_N, \mathrm{s}_N \right)
\]

where \(\mathrm{m}_N\) is the posterior mean, which is approximated by the mode of the posterior distribution, and \(\mathrm{s}_N\) is the posterior standard deviation. From the previous assignment, we have the expression for the posterior mode, which we denote here as \(\mathrm{m}_N\).

The posterior standard deviation, \(\mathrm{s}_N\), can be determined from the inverse square root of the second derivative of the log-posterior with respect to \(\mu\) at the mode:

\[
\mathrm{s}_N = \sqrt{-\left(\frac{d^2}{d\mu^2} \log p\left( \mu \mid \mathbf{x}, \sigma \right)\right)^{-1}}
\]

Substituting the expression for the second derivative from Problem 2a:

\[
\mathrm{s}_N = \sqrt{\frac{1}{\frac{N}{\sigma^2} + \frac{1}{\tau_0^2}}}
\]

This expression gives the approximate posterior standard deviation in terms of the precision, where the precision is the reciprocal of the variance.
...
The Laplace Approximation is equal to the exact posterior distribution in this specific application because the conjugacy between the Gaussian likelihood and the Gaussian prior results in a Gaussian posterior, which matches the form assumed by the Laplace Approximation. Therefore, the approximation perfectly aligns with the true posterior, making it exact rather than an approximation.
...
Transforming \(\sigma\) to \(\varphi\) using a transformation like the natural log is useful in the Laplace Approximation because it ensures the domain of the transformed variable is unconstrained, allowing for optimization and integration over the entire real line, which simplifies computations. Additionally, this transformation helps stabilize numerical procedures by reducing the skewness and scale differences in the parameter space.
...
The un-normalized joint posterior between the unknown mean, \(\mu\), and the transformed noise, \(\varphi\), is given by the probability change-of-variables formula:

\[
p\left( \mu, \varphi \mid \mathbf{x} \right) \propto \left( \prod_{n=1}^{N} \mathrm{normal}\left( x_n \mid \mu, g^{-1}(\varphi) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left( g^{-1}(\varphi) \mid \lambda \right) \times \left| \frac{d}{d\varphi} g^{-1}(\varphi) \right|
\]

Here, \(g^{-1}(\varphi)\) is the inverse transformation function that maps \(\varphi\) back to \(\sigma\), and \(\left| \frac{d}{d\varphi} g^{-1}(\varphi) \right|\) is the absolute value of the derivative of \(g^{-1}\) with respect to \(\varphi\), representing the Jacobian determinant for the change of variables.
...
The inverse transformation function that maps \(\varphi\) back to \(\sigma\) is obtained by exponentiating both sides:

\[
\sigma = g^{-1}(\varphi) = e^{\varphi}
\]

To find the derivative adjustment, we first compute the derivative of the inverse transformation function with respect to \(\varphi\):

\[
\frac{d}{d\varphi} g^{-1}(\varphi) = \frac{d}{d\varphi} e^{\varphi} = e^{\varphi}
\]

Taking the natural log of the derivative adjustment gives:

\[
\log \left| \frac{d}{d\varphi} g^{-1}(\varphi) \right| = \log \left| e^{\varphi} \right| = \varphi
\]

The log of the derivative adjustment is simply \(\varphi\), which simplifies the incorporation of this adjustment into the log-posterior.
...
```{r, solution_03d_b}
### 
test_values_1 <- c(13, 0)
log_post_1 <- my_cv_logpost(test_values_1, hw06_info)
print(log_post_1)
```
...
###
test_values_2 <- c(7, -1)
log_post_2 <- my_cv_logpost(test_values_2, hw06_info)
print(log_post_2)
```
...
map_res_01 <- optim(par = init_guess_01,
                    fn = function(p) -my_cv_logpost(p, hw06_info),  # Negate to maximize
                    gr = NULL,  # NULL since we're not providing a gradient function
                    method = "BFGS",
                    control = list(maxit = 1001),
                    hessian = TRUE)
```
...
optimal_params_01 <- map_res_01$par
optimal_params_02 <- map_res_02$par

print(optimal_params_01)
print(optimal_params_02)



hessian_01 <- map_res_01$hessian
hessian_02 <- map_res_02$hessian
...
my_laplace <- function(start_guess, logpost_func, my_info)
{
  # code adapted from the `LearnBayes`` function `laplace()`
  fit <- optim(par = start_guess,
               fn = logpost_func,  # Negate to maximize
               gr = NULL,
               my_info = my_info,
               method = "BFGS",
               hessian =  TRUE,
  ...
  mode <- fit$par  # Extract the mode from the optimization results
  post_var_matrix <- solve(-fit$hessian)  # Posterior variance matrix is the negative inverse of the Hessian
  p <- length(mode)
  # we will discuss what int means in a few weeks...
  int <- p / 2 * log(2 * pi) + 0.5 * log(det(post_var_matrix)) + logpost_func(mode, my_info)
...
# Execute the Laplace Approximation
laplace_result <- my_laplace(start_guess = c(mu0, tau0),
                              my_cv_logpost,
                              hw06_info)
...
# Extract the posterior mean and standard deviation for mu from the Laplace Approximation
posterior_mean_mu <- laplace_result$mode[1]
posterior_sd_mu <- sqrt(laplace_result$var_matrix[1, 1])

# Define the sample average calculated previously
sample_average <- 11.39  # Replace this with the actual sample average from your data

# Calculate the probability that mu is less than the sample average
probability_mu_less_than_average <- pnorm(sample_average, mean = posterior_mean_mu, sd = posterior_sd_mu)

# Print the result
print(paste("Probability that mu is less than the sample average:", probability_mu_less_than_average))
...
generate_post_samples <- function(mvn_info, num_samples) {
  MASS::mvrnorm(n = num_samples,
                mu = mvn_info$mode,  # The mean vector from Laplace Approximation
                Sigma = mvn_info$var_matrix) %>%  # The covariance matrix from Laplace Approximation
    as.data.frame() %>% tibble::as_tibble() %>% 
    purrr::set_names(c("mu", "varphi")) %>% 
    mutate(sigma = exp(varphi))  # Apply the inverse transformation to get sigma from varphi
}
...
set.seed(202004)
post_samples <-  generate_post_samples(laplace_result, num_samples = 1e4)
summary(post_samples)

posterior_mean_varphi <- mean(exp(post_samples$varphi))
posterior_mean_sigma <- mean(post_samples$sigma)  

print(paste("Posterior mean of sigma from inverse transformation:", posterior_mean_sigma))

print(summary(post_samples$sigma))
```
...
post_samples %>%
  ggplot() +
  geom_histogram(aes(x = mu), bins = 55, fill = "skyblue", color = "black") +
  theme_minimal() +
  labs(title = "Posterior Distribution of mu", x = expression(mu), y = "Frequency")
...
# Histogram for sigma
  ggplot() +
  geom_histogram(aes(x = sigma), bins = 55, fill = "lightgreen", color = "black") +
  theme_minimal() +
  labs(title = "Posterior Distribution of sigma", x = expression(sigma), y = "Frequency")
...
num_greater_than_4 <- sum(post_samples$sigma > 4)

total_samples <- nrow(post_samples)

probability_sigma_greater_than_4 <- num_greater_than_4 / total_samples

print(paste("Posterior probability that sigma is greater than 4:", probability_sigma_greater_than_4))
